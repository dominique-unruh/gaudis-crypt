...
`Program.rel p q Pre Post` is the asymmetric relational lifting: for every
pair of postconditions compatible with `Post`, the wp of `p` from a
`Pre`-related pair of states is below the wp of `q`. `Program.relE` is the
...
* Heterogeneous state and result types are supported (`Program s₁ α` vs
  `Program s₂ β`): the calculus can relate a concrete game to an abstract
...
def Program.rel {s₁ s₂ α β : Type} (p : Program s₁ α) (q : Program s₂ β)
...
def Program.relE {s₁ s₂ α β : Type} (p : Program s₁ α) (q : Program s₂ β)
    (Pre : s₁ → s₂ → Prop) (Post : α × s₁ → β × s₂ → Prop) : Prop :=
...
lemma wp_le {s₁ s₂ α β : Type} {p : Program s₁ α} {q : Program s₂ β}
...
lemma refl {s α : Type} (p : Program s α) : p.rel p Eq Eq := by
  intro F G hFG σ₁ σ₂ hpre
  subst hpre
  exact Program.wp_le_wp_of_le p F G (fun x => hFG x x rfl) σ₁
...
lemma pure_pure {s₁ s₂ α β : Type} {x₁ : α} {x₂ : β}
    ...
    (pure x₁ : Program s₁ α).rel (pure x₂ : Program s₂ β) Pre Post := by
  intro F G hFG σ₁ σ₂ hpre
  rw [wp_pure, wp_pure]
...
lemma prefix_left {s₁ s₂ α β : Type}
    {p₀ : Program s₁ Unit} {k : Program s₁ α} {q : Program s₂ β}
    {Pre Mid : s₁ → s₂ → Prop} {Post : α × s₁ → β × s₂ → Prop}
    (h₀ : p₀.rel (pure () : Program s₂ Unit) Pre (fun x y => Mid x.2 y.2))
    (h : k.rel q Mid Post) :
    (p₀ >>= fun _ => k).rel q Pre Post := by
  intro F G hFG σ₁ σ₂ hpre
  rw [wp_bind]
  calc p₀.wp (fun xτ : Unit × s₁ => k.wp F xτ.2) σ₁
      ≤ (pure () : Program s₂ Unit).wp (fun yτ : Unit × s₂ => q.wp G yτ.2) σ₂ :=
...
lemma prefix_right {s₁ s₂ α β : Type}
    {p : Program s₁ α} {q₀ : Program s₂ Unit} {k : Program s₂ β}
    {Pre Mid : s₁ → s₂ → Prop} {Post : α × s₁ → β × s₂ → Prop}
    (h₀ : (pure () : Program s₁ Unit).rel q₀ Pre (fun x y => Mid x.2 y.2))
    (h : p.rel k Mid Post) :
    p.rel (q₀ >>= fun _ => k) Pre Post := by
  intro F G hFG σ₁ σ₂ hpre
  rw [wp_bind]
  calc p.wp F σ₁
      = (pure () : Program s₁ Unit).wp (fun xτ : Unit × s₁ => p.wp F xτ.2) σ₁ := by
        rw [wp_pure]
...
lemma ite_sync {s₁ s₂ α β : Type} {c₁ c₂ : Prop} [Decidable c₁] [Decidable c₂]
    {p₁ q₁ : Program s₁ α} {p₂ q₂ : Program s₂ β}
    {Pre : s₁ → s₂ → Prop} {Post : α × s₁ → β × s₂ → Prop}
    (h_iff : c₁ ↔ c₂)
...
/-! ## Assignment rules (`Program.set` / `Program.get`) -/
...
lemma set_set {s₁ s₂ γ₁ γ₂ : Type} {L₁ : Lens γ₁ s₁} {L₂ : Lens γ₂ s₂}
    ...
    (Program.set L₁ v₁).rel (Program.set L₂ v₂) Pre Post := by
  intro F G hFG σ₁ σ₂ hpre
  rw [wp_set, wp_set]
  exact hFG _ _ (h σ₁ σ₂ hpre)
...
lemma set_left {s₁ s₂ γ β : Type} {L : Lens γ s₁} {v : γ} {x₂ : β}
    ...
    (Program.set L v).rel (pure x₂ : Program s₂ β) Pre Post := by
  intro F G hFG σ₁ σ₂ hpre
  rw [wp_set, wp_pure]
  exact hFG _ _ (h σ₁ σ₂ hpre)
...
lemma set_right {s₁ s₂ γ α : Type} {x₁ : α} {L : Lens γ s₂} {v : γ}
    ...
    (pure x₁ : Program s₁ α).rel (Program.set L v) Pre Post := by
  intro F G hFG σ₁ σ₂ hpre
  rw [wp_set, wp_pure]
  exact hFG _ _ (h σ₁ σ₂ hpre)
...
lemma get_get {s₁ s₂ γ₁ γ₂ : Type} {L₁ : Lens γ₁ s₁} {L₂ : Lens γ₂ s₂}
    ...
    (Program.get L₁).rel (Program.get L₂) Pre Post := by
  intro F G hFG σ₁ σ₂ hpre
  rw [wp_get, wp_get]
  exact hFG _ _ (h σ₁ σ₂ hpre)
...
lemma get_left {s₁ s₂ γ β : Type} {L : Lens γ s₁} {x₂ : β}
    ...
    (Program.get L).rel (pure x₂ : Program s₂ β) Pre Post := by
  intro F G hFG σ₁ σ₂ hpre
  rw [wp_get, wp_pure]
  exact hFG _ _ (h σ₁ σ₂ hpre)
...
lemma get_right {s₁ s₂ γ α : Type} {x₁ : α} {L : Lens γ s₂}
    ...
    (pure x₁ : Program s₁ α).rel (Program.get L) Pre Post := by
  intro F G hFG σ₁ σ₂ hpre
  rw [wp_get, wp_pure]
  exact hFG _ _ (h σ₁ σ₂ hpre)
...
lemma uniform_bij {s₁ s₂ α β : Type} [Fintype α] [Nonempty α] [Fintype β] [Nonempty β]
    ...
    (Program.uniform : Program s₁ α).rel (Program.uniform : Program s₂ β) Pre Post := by
  intro F G hFG σ₁ σ₂ hpre
...
lemma sample_left {s₁ s₂ α β₁ β₂ : Type} [Fintype α] [Nonempty α]
    {k : α → Program s₁ β₁} {q : Program s₂ β₂}
    {Pre : s₁ → s₂ → Prop} {Post : β₁ × s₁ → β₂ × s₂ → Prop}
    (h : ∀ v, (k v).rel q Pre Post) :
    ((Program.uniform : Program s₁ α) >>= k).rel q Pre Post := by
  intro F G hFG σ₁ σ₂ hpre
  ...
  have hsum : ((Program.uniform : Program s₁ α) >>= k).wp F σ₁
...
lemma sample_right {s₁ s₂ α β₁ β₂ : Type} [Fintype α] [Nonempty α]
    {p : Program s₁ β₁} {k : α → Program s₂ β₂}
    {Pre : s₁ → s₂ → Prop} {Post : β₁ × s₁ → β₂ × s₂ → Prop}
    (h : ∀ v, p.rel (k v) Pre Post) :
    p.rel ((Program.uniform : Program s₂ α) >>= k) Pre Post := by
  intro F G hFG σ₁ σ₂ hpre
  have hc0 : (Fintype.card α : ENNReal) ≠ 0 := by
    exact_mod_cast (Fintype.card_ne_zero : Fintype.card α ≠ 0)
  have hct : (Fintype.card α : ENNReal) ≠ ⊤ := ENNReal.natCast_ne_top _
  have hsum : ((Program.uniform : Program s₂ α) >>= k).wp G σ₂
...
lemma wp_eq {s₁ s₂ α β : Type} {p : Program s₁ α} {q : Program s₂ β}
    {Pre : s₁ → s₂ → Prop} {Post : α × s₁ → β × s₂ → Prop}
    (h : p.relE q Pre Post)
    {F : α × s₁ → ENNReal} {G : β × s₂ → ENNReal}
    (hFG : ∀ x y, Post x y → F x = G y)
...
/-- Reflexivity. -/
lemma refl {s α : Type} (p : Program s α) : p.relE p Eq Eq :=
  ⟨Program.rel.refl p,
   (Program.rel.refl p).conseq (fun _ _ h => h.symm) (fun _ _ h => h.symm)⟩

/-- Program equality gives the diagonal `relE`. -/
lemma of_eq {s α : Type} {p q : Program s α} (h : p = q) :
    p.relE q Eq Eq := by
  subst h
  exact Program.relE.refl p

/-- Symmetry (with flipped relations). -/
lemma symm {s₁ s₂ α β : Type} {p : Program s₁ α} {q : Program s₂ β}
    {Pre : s₁ → s₂ → Prop} {Post : α × s₁ → β × s₂ → Prop}
    (h : p.relE q Pre Post) :
...
lemma pure_pure {s₁ s₂ α β : Type} {x₁ : α} {x₂ : β}
    {Pre : s₁ → s₂ → Prop} {Post : α × s₁ → β × s₂ → Prop}
    (h : ∀ σ₁ σ₂, Pre σ₁ σ₂ → Post (x₁, σ₁) (x₂, σ₂)) :
    (pure x₁ : Program s₁ α).relE (pure x₂ : Program s₂ β) Pre Post :=
  ⟨Program.rel.pure_pure h, Program.rel.pure_pure (fun σ₂ σ₁ hpre => h σ₁ σ₂ hpre)⟩
...
lemma bind {s₁ s₂ α₁ α₂ β₁ β₂ : Type}
  ...
  ⟨Program.rel.bind h_p.1 (fun x₁ x₂ => (h_k x₁ x₂).1),
   Program.rel.bind h_p.2 (fun x₂ x₁ => (h_k x₁ x₂).2)⟩
...
lemma set_set {s₁ s₂ γ₁ γ₂ : Type} {L₁ : Lens γ₁ s₁} {L₂ : Lens γ₂ s₂}
    ...
    {Pre : s₁ → s₂ → Prop} {Post : Unit × s₁ → Unit × s₂ → Prop}
    (h : ∀ σ₁ σ₂, Pre σ₁ σ₂ → Post ((), L₁.set v₁ σ₁) ((), L₂.set v₂ σ₂)) :
    (Program.set L₁ v₁).relE (Program.set L₂ v₂) Pre Post :=
  ⟨Program.rel.set_set h, Program.rel.set_set (fun σ₂ σ₁ hpre => h σ₁ σ₂ hpre)⟩

/-- Two-sided `get` for `relE`. -/
lemma get_get {s₁ s₂ γ₁ γ₂ : Type} {L₁ : Lens γ₁ s₁} {L₂ : Lens γ₂ s₂}
    {Pre : s₁ → s₂ → Prop} {Post : γ₁ × s₁ → γ₂ × s₂ → Prop}
    (h : ∀ σ₁ σ₂, Pre σ₁ σ₂ → Post (L₁.get σ₁, σ₁) (L₂.get σ₂, σ₂)) :
    (Program.get L₁).relE (Program.get L₂) Pre Post :=
  ⟨Program.rel.get_get h, Program.rel.get_get (fun σ₂ σ₁ hpre => h σ₁ σ₂ hpre)⟩
...
lemma uniform_bij {s₁ s₂ α β : Type} [Fintype α] [Nonempty α] [Fintype β] [Nonempty β]
    ...
    {Pre : s₁ → s₂ → Prop} {Post : α × s₁ → β × s₂ → Prop}
    (h : ∀ v σ₁ σ₂, Pre σ₁ σ₂ → Post (v, σ₁) (e v, σ₂)) :
    (Program.uniform : Program s₁ α).relE (Program.uniform : Program s₂ β) Pre Post := by
  refine ⟨Program.rel.uniform_bij e h, Program.rel.uniform_bij e.symm ?_⟩
  intro v σ₂ σ₁ hpre
...
lemma ite_sync {s₁ s₂ α β : Type} {c₁ c₂ : Prop} [Decidable c₁] [Decidable c₂]
  ...
  ⟨Program.rel.ite_sync h_iff (fun h1 h2 => (h_t h1 h2).1) (fun h1 h2 => (h_f h1 h2).1),
