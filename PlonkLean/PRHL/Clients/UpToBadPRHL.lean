...
/-!
# Validation client 2: the identical-until-bad hop, relationally

A relational re-proof of the `UpToBad.lean` core: `ow_game_1_tracked` and
`ow_game_2_tracked` differ only in whether the random oracle is
pre-programmed at `chal_x`; their win events agree until the adversary
queries `chal_x` (the `chal_x_queried_gh` flag).

**One judgment delivers everything.** We prove a single `relE` judgment
`game_relE : G1 ~ G2 : Eq ⇒ PostG` with

```
PostG u v := flag u = flag v
           ∧ (flag u = false → u.1 = v.1 ∧ ∃ y, u.2 = insert_at_chal_x y v.2)
```

and obtain all three theorems of the unary development as corollaries:
the agree-until-bad wp equality (`relE.wp_eq`), the bad-event probability
equality (`relE.bad_eq` — the unary proof needed a 138-line mass-
conservation/cancellation chain), and the up-to-bad hop bound
(`relE.up_to_bad`).

The coupling invariant carried through the adversary loop is
...
— on good runs the Game-1 state *is* the Game-2 state with `RO[x ↦ y]`
overwritten; after bad the runs decouple and only the flags are related
(the rectangular rule `rel.of_unary`, whose side conditions are the
flag-monotonicity and mass-1 facts — the part of the unary development
that genuinely survives).

**Hypothesis delta vs the unary original**: the single-judgment route
needs the adversary's mass-1 hypothesis for *all* corollaries (the
rectangular phase needs the right side to be lossless), whereas the unary
`until_bad`/hop lemmas avoid it by working only at bad-vanishing posts.
The unary `bad_eq` needs it either way, and so does the OW master theorem,
...
private lemma lqt_flag_zero {inp : input} {F : output × state → ENNReal}
  ...
  refine Program.wp_zero_of_lens_preserves h_lq ?_ h
  intro u hu
...
private lemma lazy_query_mass (inp : input) (σ : state) :
  ...
  | none =>
    rw [wp_lq_miss inp h]
    have := Program.uniform_mass_one (s := state) (α := output) σ
...
private lemma body_flag_zero
  ...
  refine Program.wp_zero_of_lens_preserves h_flag ?_ h
  rintro ⟨u, s⟩ hu
  change (lazy_query_tracked (oracle_input.get s)).wp _ s = 0
  refine lqt_flag_zero ?_ hu
  rintro ⟨a, s'⟩ hw
  change (Program.set oracle_output a).wp F s' = 0
  rw [wp_set]
  refine hF _ ?_
...
private lemma body_mass
  ...
  change (ow_adv >>= fun _ =>
    Program.get oracle_input >>= fun inp =>
    lazy_query_tracked inp >>= fun y' =>
    Program.set oracle_output y').wp (fun _ => (1 : ENNReal)) σ = 1
  rw [wp_bind]
  refine (wp_post_congr _ ?_ σ).trans (h_mass σ)
  rintro ⟨u, s⟩
  change (Program.get oracle_input >>= fun inp =>
    lazy_query_tracked inp >>= fun y' =>
    Program.set oracle_output y').wp (fun _ => (1 : ENNReal)) s = (1 : ENNReal)
  rw [wp_bind, wp_get]
  dsimp only
  rw [wp_bind]
  refine (wp_post_congr _ ?_ s).trans (lqt_mass _ s)
  rintro ⟨a, s'⟩
  change (Program.set oracle_output a).wp (fun _ => (1 : ENNReal)) s' = (1 : ENNReal)
  rw [wp_set]

/-- The verification segment shared by both games (parameterized by the
    challenge output `y_v` it compares against). -/
noncomputable def finalSeg (y_v : output) : Program state Bool :=
...
private lemma final_flag_zero (y_v : output) {F : Bool × state → ENNReal}
  ...
  rintro ⟨a, s'⟩ hw
  change (pure (decide (a = y_v)) : Program state Bool).wp F s' = 0
  rw [wp_pure]
  exact hF _ hw
...
private lemma final_mass (y_v : output) (σ : state) :
  ...
  rintro ⟨a, s'⟩
  change (pure (decide (a = y_v)) : Program state Bool).wp
...
private lemma body_good_relE
  ...
  have h_adv : ow_adv.relE ow_adv (GoodPre x y)
    ...
    have h2 := Program.relE.frame ow_challenge_x ow_challenge_x h_cx h_cx
      (Program.relE.frame chal_x_queried_gh chal_x_queried_gh h_flag h_flag
        ((Program.relE.self_shift h_RO (insRO_mem x y)).symm) false false)
      x x
  ...
  · refine Program.relE.bind
      ...
      (fun y₁ y₂ => Program.relE.set_set ?_)
...
private lemma body_relE
  ...
  (Program.relE.or_pre (body_good_relE ow_adv h_RO h_flag h_cx x y)
...
private lemma final_good_relE (y_v : output) (x : input) (y : output) :
  ...
  refine Program.relE.bind
    (Mid := fun u v : input × state => u.1 = v.1 ∧ GoodPre x y u.2 v.2)
    (Program.relE.get_get ?_) (fun r₁ r₂ => ?_)
  · rintro σ₁ σ₂ ⟨hf₂, hcx, rfl⟩
    exact ⟨by rw [get_insRO], hf₂, hcx, rfl⟩
  · refine Program.relE.bind
      (Mid := fun u v : output × state => InvUB x y u.2 v.2
      ...
      (fun yc₁ yc₂ => Program.relE.pure_pure ?_)
    · rintro τ₁ τ₂ ⟨hval, hf₂, hcx, hst⟩
      exact ⟨⟨hval, hf₂, hcx⟩, hst⟩
    · rintro τ₁ τ₂ ⟨⟨hfeq, hgood⟩, hval⟩
      refine ⟨hfeq, ?_⟩
...
private lemma final_relE (y_v : output) (x : input) (y : output) :
  ...
  (Program.relE.or_pre (final_good_relE y_v x y) (final_bad_relE y_v)).conseq
    (fun _ _ h => invUB_cases h) (fun _ _ h => h)
...
theorem ow_game_tracked_relE
  ...
  have hTail : ∀ (x : input) (y : output),
      ...
      (fun u v => chal_x_queried_gh.get u.2 = chal_x_queried_gh.get v.2
        ∧ (chal_x_queried_gh.get u.2 = false →
            u.1 = v.1 ∧ ∃ y', u.2 = insert_at_chal_x y' v.2)) :=
    fun x y => Program.relE.bind
      (Mid := fun u v : Unit × state => InvUB x y u.2 v.2)
      (Program.relE.loop_n (body_relE ow_adv h_RO h_flag h_cx h_mass x y) q)
...
theorem ow_game_1_tracked_eq_ow_game_2_tracked_until_bad_prhl
    (h_RO : ow_adv.inRange random_oracle_state.compl.range)
    (h_cx : ow_adv.inRange ow_challenge_x.compl.range)
    (h_flag : ow_adv.inRange chal_x_queried_gh.compl.range)
    (h_mass : ∀ σ, ow_adv.wp (fun _ => (1 : ENNReal)) σ = 1)
...
theorem ow_game_1_tracked_bad_eq_ow_game_2_tracked_bad_prhl
    (h_RO : ow_adv.inRange random_oracle_state.compl.range)
    (h_cx : ow_adv.inRange ow_challenge_x.compl.range)
    (h_flag : ow_adv.inRange chal_x_queried_gh.compl.range)
    (h_mass : ∀ σ, ow_adv.wp (fun _ => (1 : ENNReal)) σ = 1)
...
theorem ow_game_1_tracked_le_ow_game_2_tracked_plus_bad_prhl
    (h_RO : ow_adv.inRange random_oracle_state.compl.range)
    (h_cx : ow_adv.inRange ow_challenge_x.compl.range)
    (h_flag : ow_adv.inRange chal_x_queried_gh.compl.range)
    (h_mass : ∀ σ, ow_adv.wp (fun _ => (1 : ENNReal)) σ = 1)
