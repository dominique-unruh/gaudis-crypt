...
inductive ModuleExpression [ProgramSpec] : ModuleContext → ModuleType → Type _ where
  | proc {sig} : Procedure sig → ModuleExpression Δ (.proc sig)
  -- TODO: add:
  -- | func {holeSigs} {sig} : ProcedureWithHoles holeSigs sig → ModuleExpression Δ (something something .proc sig)
...
mutual
  ...
  inductive Normal [ProgramSpec] : ModuleExpression Δ T → Prop where
    | neutral : Neutral e → Normal e
    | abs     {body : ModuleExpression (ModuleContext.append Δ A) B} : Normal body → Normal (.abs body)
    | pair    {a : ModuleExpression Δ A} {b : ModuleExpression Δ B} : Normal a → Normal b → Normal (.pair a b)
    | const   : Normal (.proc p)
  ...
  inductive Neutral [ProgramSpec] : ModuleExpression Δ T → Prop where
    | var : Neutral (.var r)
...
private def decidableNormalNeutral [ProgramSpec] (m : ModuleExpression Δ t) :
...
instance [ProgramSpec] (m : ModuleExpression Γ A) : Decidable (Normal m) := (decidableNormalNeutral m).1
instance [ProgramSpec] (m : ModuleExpression Γ A) : Decidable (Neutral m) := (decidableNormalNeutral m).2
...
inductive NormalClosed [ProgramSpec] : ModuleExpression .empty T → Prop where
  | const : NormalClosed (.proc p)
  | abs   {body : ModuleExpression (ModuleContext.append .empty A) B} : Normal body → NormalClosed (.abs body)
  | pair  {a : ModuleExpression .empty A} {b : ModuleExpression .empty B} :
      NormalClosed a → NormalClosed b → NormalClosed (.pair a b)

lemma not_neutral_empty [ProgramSpec] {T : ModuleType} : {m : ModuleExpression .empty T} → ¬ Neutral m
  | .var r,   _       => nomatch r
  | .app _ _, .app nf _ => not_neutral_empty nf
  | .fst _,   .fst ne   => not_neutral_empty ne
  | .snd _,   .snd ne   => not_neutral_empty ne

/-- Extends a renaming `ρ : ModuleContextIdx Δ → ModuleContextIdx Γ` to work under one binder of type `A`.
...
private def liftRename {Δ Γ : ModuleContext} {A : ModuleType}
    (ρ : ∀ {T}, ModuleContextIdx Δ T → ModuleContextIdx Γ T) {T} : ModuleContextIdx (ModuleContext.append Δ A) T → ModuleContextIdx (ModuleContext.append Γ A) T
  | .zero   => .zero
  | .succ r => .succ (ρ r)

/-- Applies a renaming `ρ : ModuleContextIdx Δ → ModuleContextIdx Γ` to every variable in a term, producing a term
...
private def ModuleExpression.rename [ProgramSpec] (ρ : ∀ {T}, ModuleContextIdx Δ T → ModuleContextIdx Γ T) : ModuleExpression Δ T → ModuleExpression Γ T
  | proc p  => proc p
  | .var r    => .var (ρ r)
...
/-- Extends a simultaneous substitution `σ : ModuleContextIdx Δ → ModuleExpression Γ` to work under one binder
    of type `A`, yielding a substitution `ModuleContextIdx (Δ,A) → ModuleExpression (Γ,A)`.
...
private def liftSubst [ProgramSpec] {Δ Γ : ModuleContext} {A : ModuleType}
    (σ : ∀ {T}, ModuleContextIdx Δ T → ModuleExpression Γ T) {T} : ModuleContextIdx (ModuleContext.append Δ A) T → ModuleExpression (ModuleContext.append Γ A) T
  | .zero   => .var .zero
  | .succ r => (σ r).rename (fun {_} r => .succ r)

/-- Applies a simultaneous substitution `σ : ModuleContextIdx Δ → ModuleExpression Γ` to every variable in a
    term, producing a term over context `Γ`.  Goes under binders by lifting `σ` with
    `liftSubst`. -/
private def substSimultaneous [ProgramSpec] (σ : ∀ {T}, ModuleContextIdx Δ T → ModuleExpression Γ T) : ModuleExpression Δ T → ModuleExpression Γ T
  | .proc p  => .proc p
  | .var r    => σ r
...
private def substVar [ProgramSpec] {Δ : ModuleContext} {u : ModuleType} (arg : ModuleExpression Δ u) {T} :
...
def subst [ProgramSpec] (body : ModuleExpression (Δ.append u) t) (arg : ModuleExpression Δ u) : ModuleExpression Δ t :=
  substSimultaneous (substVar arg) body

scoped instance instModuleExpressionSTLCspec [ProgramSpec] : Metatheory.STLCext.STLCspec where
  baseTypes := ProcedureSignature
  baseTypeValue := Procedure

def moduleTypeToSTLC [ProgramSpec] : ModuleType → Metatheory.STLCext.Ty
| .prod A B => .prod (moduleTypeToSTLC A) (moduleTypeToSTLC B)
| .arr A B => .arr (moduleTypeToSTLC A) (moduleTypeToSTLC B)
| .proc sig => .base sig

def moduleExpressionToSTLC [ProgramSpec] : ModuleExpression Γ T → Metatheory.STLCext.Term
  | .proc p => .value p
  | .var n => .var n.toNat
...
def moduleContextToSTLC [ProgramSpec] : ModuleContext → Metatheory.STLCext.Context
| .empty => []
| .append Γ T => moduleTypeToSTLC T :: moduleContextToSTLC Γ

theorem moduleExpressionToSTLC_hasType [ProgramSpec] (m : ModuleExpression Γ T) :
  Metatheory.STLCext.HasType (moduleContextToSTLC Γ) (moduleExpressionToSTLC m) (moduleTypeToSTLC T) := by induction m with
  | proc c =>
...
def isPair [ProgramSpec] (m : ModuleExpression Δ (.prod T U)) : Prop :=
  match h : m with | .pair _ _ => true | _ => false

instance [ProgramSpec] (m : ModuleExpression Δ (.prod T U)) : Decidable (isPair m) :=
  match m with
  | .pair _ _ => isTrue rfl
  | .var _ | .app _ _ | .fst _ | .snd _ => isFalse Bool.false_ne_true

def isPair.split[ProgramSpec] {m : ModuleExpression Δ (.prod T U)} (_: isPair m) : (ModuleExpression Δ T × ModuleExpression Δ U) :=
  match m with
   | .pair m1 m2 => (m1,m2)
...
def isPair.fst [ProgramSpec] {m : ModuleExpression Δ (.prod T U)} (h: isPair m) : ModuleExpression Δ T :=
  h.split.1

def isPair.snd [ProgramSpec] {m : ModuleExpression Δ (.prod T U)} (h: isPair m) : ModuleExpression Δ U :=
  h.split.2

def isAbs [ProgramSpec] (m : ModuleExpression Δ T) : Prop :=
  match h : m with | .abs _ => true | _ => false

instance [ProgramSpec] (m : ModuleExpression Δ T) : Decidable (isAbs m) :=
  match m with
  | .abs _ => isTrue rfl
  | .var _ | .pair _ _ | .fst _ | .snd _ | .app _ _ | .proc _ => isFalse Bool.false_ne_true

def isAbs.body [ProgramSpec] {m : ModuleExpression Δ (.arr T U)} (_ : isAbs m) :
    ModuleExpression (Δ.append T) U :=
  match m with
   | .abs body => body
   | .fst _ => False.elim (by simp [isAbs] at *)
   | .snd _ => False.elim (by simp [isAbs] at *)
   | .var _ => False.elim (by simp [isAbs] at *)
   | .app _ _ => False.elim (by simp [isAbs] at *)


def cbvReductionStep [ProgramSpec] (m : ModuleExpression Δ t) (nn : ¬ Normal m) :
  ...
  | .app hd arg =>
      if abs : isAbs hd then
        ...
        if nn_hd : ¬ Normal hd then
          ...
          have nn_arg : ¬ Normal arg := fun ha => nn (.neutral (.app (match not_not.mp nn_hd with
                                        | .neutral ne => ne | .abs _ => absurd rfl abs) ha))
...
inductive ReductionStep [ProgramSpec] : ModuleExpression Δ T → ModuleExpression Δ T → Prop where
...
private lemma moduleExpressionToSTLC_rename_shift [ProgramSpec] (d : Nat)
  ...
  induction m with
  | proc p => intros; simp [ModuleExpression.rename, moduleExpressionToSTLC, Metatheory.STLCext.Term.shift]
  | var r =>
    intro c Γ ρ hlo hhi
  ...
  | fst e ih =>
    intro c Γ ρ hlo hhi
    simp [ModuleExpression.rename, moduleExpressionToSTLC, Metatheory.STLCext.Term.shift, ih c ρ hlo hhi]
  | snd e ih =>
    intro c Γ ρ hlo hhi
    simp [ModuleExpression.rename, moduleExpressionToSTLC, Metatheory.STLCext.Term.shift, ih c ρ hlo hhi]
  | pair a b iha ihb =>
  ...
  | abs body ih =>
    ...
    · intro T r hr
      cases r with
      | zero => simp [liftRename, ModuleContextIdx.toNat] at hr
      | succ r' =>
        simp only [liftRename, ModuleContextIdx.toNat] at *
        have := hhi r' (by omega); omega

private lemma moduleExpressionToSTLC_substAll_level [ProgramSpec]
  ...
  induction m with
  | proc p => intros; simp [substSimultaneous, moduleExpressionToSTLC, Metatheory.STLCext.Term.subst]
  | var r =>
    intro k Γ σ hσ
    simp [substSimultaneous, moduleExpressionToSTLC, hσ]
  | app f a ihf iha =>
    intro k Γ σ hσ
    simp [substSimultaneous, moduleExpressionToSTLC, Metatheory.STLCext.Term.subst, ihf k σ hσ, iha k σ hσ]
  | fst e ih =>
    intro k Γ σ hσ
  ...
  | pair a b iha ihb =>
    intro k Γ σ hσ
    simp [substSimultaneous, moduleExpressionToSTLC, Metatheory.STLCext.Term.subst, iha k σ hσ, ihb k σ hσ]
  | abs body ih =>
    intro k Γ σ hσ
    ...
    cases r with
    | zero =>
      simp only [liftSubst, moduleExpressionToSTLC, ModuleContextIdx.toNat, Metatheory.STLCext.Term.subst]
...
private lemma moduleExpressionToSTLC_subst [ProgramSpec]
  ...
  · intro T r
    cases r with
    | zero =>
      simp only [substVar, moduleExpressionToSTLC, ModuleContextIdx.toNat, Metatheory.STLCext.Term.subst]
      simp [Metatheory.STLCext.Term.shift_zero]
    | succ r' =>
      simp only [substVar, moduleExpressionToSTLC, ModuleContextIdx.toNat, Metatheory.STLCext.Term.subst,
...
private theorem reduce_acc [ProgramSpec] {Γ : ModuleContext} {T : ModuleType} (m : ModuleExpression Γ T) :
  ...
  suffices h : ∀ M : Metatheory.STLCext.Term, Metatheory.STLCext.SN M →
      ∀ (n : ModuleExpression Γ T), moduleExpressionToSTLC n = M →
      Acc (fun p q => Metatheory.STLCext.Step (moduleExpressionToSTLC q) (moduleExpressionToSTLC p)) n from
...
scoped instance (priority := 1001) [ProgramSpec] {Γ : ModuleContext} {T : ModuleType} :
    WellFoundedRelation (ModuleExpression Γ T) :=
  ⟨fun p q => Metatheory.STLCext.Step (moduleExpressionToSTLC q) (moduleExpressionToSTLC p), ⟨reduce_acc⟩⟩

theorem cbvReductionStep_preservation
 [ProgramSpec] {R : ∀ {Γ T}, ModuleExpression Γ T → ModuleExpression Γ T → Prop}
...
theorem cbvReductionStep_is_reductionStep [ProgramSpec] (m : ModuleExpression Γ T) (nn : ¬ Normal m) :
...
private lemma ModuleContextIdx.toNat_inj' {Γ : ModuleContext} :
  ...
  | _, _, .succ r1', .succ r2', h => by
      simp [ModuleContextIdx.toNat] at h
...
private lemma ModuleContextIdx.toNat_inj {Γ : ModuleContext} {T : ModuleType} :
  ...
  | .succ r, .succ r', h => congrArg ModuleContextIdx.succ (ModuleContextIdx.toNat_inj r r' (Nat.succ.inj h))

def ModuleExpression.erasedEqual
  [ProgramSpec] (m : ModuleExpression Γ T) (m' : ModuleExpression Γ' T') : Prop := match m, m' with
  | @ModuleExpression.proc _ _ sig p, @ModuleExpression.proc _ _ sig' p' => sig = sig' ∧ HEq p p'
...
theorem moduleExpressionToSTLC_injective [ProgramSpec] {Γ Γ' : ModuleContext} {T T' : ModuleType}
  ...
  induction m with
  | proc p =>
    intro Γ' T' m' h; cases m' <;> simp_all [moduleExpressionToSTLC, ModuleExpression.erasedEqual]
  | var r =>
    intro Γ' T' m' h; cases m' <;> simp_all [moduleExpressionToSTLC, ModuleExpression.erasedEqual]
  | app f a ihf iha =>
    intro Γ' T' m' h
    ...
    | _ => simp [moduleExpressionToSTLC] at h
  | fst e ih =>
    intro Γ' T' m' h
    cases m' with
    | fst e' =>
...
theorem ModuleExpression.erasedEqual_refl [ProgramSpec] (m : ModuleExpression Γ T) : ModuleExpression.erasedEqual m m := by
  induction m with
...
theorem ModuleExpression.erasedEqual_pair_right [ProgramSpec] (a : ModuleExpression Γ T)
...
theorem ModuleExpression.erasedEqual_pair_left [ProgramSpec] {a : ModuleExpression Γ T} {a' : ModuleExpression Γ T'} (b : ModuleExpression Γ U)
    (h : ModuleExpression.erasedEqual a a') :
    ModuleExpression.erasedEqual (.pair a b) (.pair a' b) :=
  ⟨h, ModuleExpression.erasedEqual_refl b⟩

theorem reductionStep_stlc_compat [ProgramSpec] (m m' : ModuleExpression Γ T) :
    ReductionStep m m' → Metatheory.STLCext.Step (moduleExpressionToSTLC m) (moduleExpressionToSTLC m') := by
  intro h
  induction h with
  | beta =>
      simp only [moduleExpressionToSTLC]
...
theorem reductionStep_stlc_complete [ProgramSpec]
  ...
  | proc _ => simp only [moduleExpressionToSTLC] at h; cases h
  | var _ => simp only [moduleExpressionToSTLC] at h; cases h
  | app f arg ihf iharg =>
...
theorem cbvReductionStep_stlc_compat [ProgramSpec] (m : ModuleExpression Γ T) (nn : ¬ Normal m) :
  ...
  | fst m' ih =>
    ...
    · cases m' with
      | pair m1 m2 =>
        simp [isPair.fst, isPair.split, moduleExpressionToSTLC]
  ...
  | snd m' ih =>
    ...
    · cases m' with
      | pair m1 m2 =>
        simp [isPair.snd, isPair.split, moduleExpressionToSTLC]
...
def reduce [ProgramSpec] (m : ModuleExpression Γ T) : ModuleExpression Γ T :=
...
def MultiStepReduction [ProgramSpec] : ModuleExpression Γ T → ModuleExpression Γ T → Prop :=
  Rewriting.Star ReductionStep

theorem multiStepReduction_reduce [ProgramSpec] {m : ModuleExpression Γ T} :
  ...
  · exact Rewriting.Star.head (cbvReductionStep_is_reductionStep n h) (ih _ (cbvReductionStep_stlc_compat n h))

theorem multiStepReduction_to_stlc_star [ProgramSpec] {m m' : ModuleExpression Γ T}
    (h : MultiStepReduction m m') :
    Rewriting.Star Metatheory.STLCext.Step (moduleExpressionToSTLC m) (moduleExpressionToSTLC m') := by
  induction h with
  | refl => exact Rewriting.Star.refl _
  | tail hab hbc ih => exact Rewriting.Star.tail ih (reductionStep_stlc_compat _ _ hbc)

theorem moduleExpressionToSTLC_Normal_iff [ProgramSpec] {m : ModuleExpression Γ T} :
    ...
    | app f arg ihf iharg =>
      ...
      · intro hn N h
        ...
        cases hn with | neutral hne => cases hne with | app hf_n harg_n =>
          ...
          | funcApp => cases f <;> simp [moduleExpressionToSTLC] at hF
...
theorem multiStepReduction_stlc_complete [ProgramSpec] (m : ModuleExpression Γ T) {M' : Metatheory.STLCext.Term}
...
private theorem ModuleExpression.erasedEqual_normal_neutral_eq [ProgramSpec] {Γ : ModuleContext} {T1 T2 : ModuleType}
    (m : ModuleExpression Γ T1) (m' : ModuleExpression Γ T2) (h : ModuleExpression.erasedEqual m m') :
...
theorem ModuleExpression.erasedEqual_normal_eq [ProgramSpec] {Γ : ModuleContext} {T : ModuleType}
...
theorem ModuleExpression.erasedEqual_neutral_eq [ProgramSpec] {Γ : ModuleContext} {T1 T2 : ModuleType}
...
theorem moduleExpressionToSTLC_injective_normal [ProgramSpec] {Γ : ModuleContext} {T : ModuleType}
    {n1 n2 : ModuleExpression Γ T} (hn1 : Normal n1)
...
theorem reduce_normal [ProgramSpec] (m : ModuleExpression Δ t) : Normal (reduce m) := by
...
theorem confluence [ProgramSpec] {m m1 m2 : ModuleExpression Γ T}
   (h1 : MultiStepReduction m m1) (h2 : MultiStepReduction m m2) :
   reduce m1 = reduce m2 := by
  have star1 : Rewriting.Star Metatheory.STLCext.Step (moduleExpressionToSTLC m) (moduleExpressionToSTLC (reduce m1)) :=
    Rewriting.Star.trans
      (multiStepReduction_to_stlc_star h1) (multiStepReduction_to_stlc_star multiStepReduction_reduce)
  have star2 : Rewriting.Star Metatheory.STLCext.Step (moduleExpressionToSTLC m) (moduleExpressionToSTLC (reduce m2)) :=
    Rewriting.Star.trans
      (multiStepReduction_to_stlc_star h2) (multiStepReduction_to_stlc_star multiStepReduction_reduce)
...
private lemma Normal.to_normalClosed [ProgramSpec] {T : ModuleType} {m : ModuleExpression .empty T} :
...
theorem reduce_normalClosed [ProgramSpec] (m : ModuleExpression .empty t) : NormalClosed (reduce m) :=
  (reduce_normal m).to_normalClosed

structure Module [ProgramSpec] (T : ModuleType) where
  expression : ModuleExpression .empty T
  normal : NormalClosed expression

def ModuleExpression.toProcModule [ProgramSpec] {T : ModuleType}
  (m : ModuleExpression .empty T) : Module T :=
  ⟨reduce m, reduce_normalClosed m⟩

instance [ProgramSpec] : CoeFun (Module (.arr T U)) (fun _ ↦ Module T → Module U) where
  coe f x := ModuleExpression.toProcModule (ModuleExpression.app f.expression x.expression)

@[simp]
theorem reduce_fst [ProgramSpec] (m : ModuleExpression Γ T) (m' : ModuleExpression Γ T') :
...
@[simp]
theorem reduce_snd [ProgramSpec] (m : ModuleExpression Γ T) (m' : ModuleExpression Γ T') :
...
@[simp]
theorem reduce_idempotent [ProgramSpec] (m : ModuleExpression Γ T) :
  reduce (reduce m) = reduce m := by
  conv_lhs => unfold reduce
  simp [reduce_normal m]

theorem multiStepReduction_app [ProgramSpec]
...
theorem reduce_app [ProgramSpec] (m : ModuleExpression Γ (.arr T U)) (m' : ModuleExpression Γ T) :
...
@[simp]
theorem reduce_beta [ProgramSpec]
...
private theorem multiStepReduction_fst [ProgramSpec]
...
-- => Investigate @[gcongr]
-- @[congr]
theorem reduce_fst_cong [ProgramSpec] (m m' : ModuleExpression Γ (.prod T U)) :
...
def Module.fst [ProgramSpec] {T U} (m : Module (.prod T U)) : Module T :=
  m.expression.fst.toProcModule

def Module.snd [ProgramSpec] {T U} (m : Module (.prod T U)) : Module T :=
  m.expression.fst.toProcModule

def Module.pair [ProgramSpec] {T U} (m1 : Module T) (m2 : Module U) : Module (.prod T U) :=
  (m1.expression.pair m2.expression).toProcModule

@[ext]
theorem Module.ext [ProgramSpec] {T} {m1 m2 : Module T} (h : m1.expression = m2.expression) :
  m1 = m2 := by
  obtain ⟨e1, n1⟩ := m1; obtain ⟨e2, n2⟩ := m2
  simp only at h; subst h; rfl

@[simp]
theorem Module.expression_fst [ProgramSpec] {T U} (m : Module (.prod T U)) :
    m.fst.expression = reduce m.expression.fst := rfl

@[simp]
theorem Module.toProcModule_expression [ProgramSpec] {T} (m : ModuleExpression .empty T) :
    (ModuleExpression.toProcModule m).expression = reduce m := rfl

@[simp]
theorem reduce_fst_pair [ProgramSpec] {T U} (m1 : ModuleExpression Γ T) (m2 : ModuleExpression Γ U) :
...
private theorem multiStepReduction_pair [ProgramSpec]
...
@[simp]
theorem reduce_pair [ProgramSpec] {T U} (m1 : ModuleExpression Γ T) (m2 : ModuleExpression Γ U) :
    reduce (ModuleExpression.pair m1 m2) = ModuleExpression.pair (reduce m1) (reduce m2) := by
  have h2 : MultiStepReduction (ModuleExpression.pair m1 m2) (ModuleExpression.pair (reduce m1) (reduce m2)) :=
    multiStepReduction_pair multiStepReduction_reduce multiStepReduction_reduce
  have key : reduce (ModuleExpression.pair m1 m2) = reduce (ModuleExpression.pair (reduce m1) (reduce m2)) :=
...
@[simp]
theorem Module.reduce_expression [ProgramSpec] {T} (m : Module T) : reduce m.expression = m.expression := by
...
@[simp]
theorem Module.fst_pair [ProgramSpec] {T U} (m1 : Module T) (m2 : Module U) :
...
opaque FV : Type
-- Placeholder
axiom fv_proc [ProgramSpec] {sig} (proc : Procedure sig) : Set FV

def fv' [ProgramSpec] {mctx : ModuleContext} {mty : ModuleType} (mex : ModuleExpression mctx mty) : Set FV :=
  match mex with
    | .proc p => fv_proc p
    | .var .zero => {}
    | .var (.succ n) => {}
...
theorem fv'_app [ProgramSpec] (a : ModuleExpression Γ (ModuleType.arr A B)) (b : ModuleExpression Γ A) :
  fv' (ModuleExpression.app a b) = fv' a ∪ fv' b := rfl

def fv [ProgramSpec] (m : ModuleExpression c t) : Set FV := fv' (reduce m)

private lemma fv'_var_eq [ProgramSpec] {Γ T} (r : ModuleContextIdx Γ T) : fv' (.var r) = {} := by
  cases r <;> rfl

private lemma fv'_rename_eq [ProgramSpec] {Δ U} (m : ModuleExpression Δ U) :
    ∀ {Γ : ModuleContext} (ρ : ∀ {T}, ModuleContextIdx Δ T → ModuleContextIdx Γ T), fv' (m.rename ρ) = fv' m := by
  induction m with
  | proc => intros; simp [ModuleExpression.rename, fv']
  | var r => intros Γ ρ; simp [ModuleExpression.rename, fv'_var_eq]
...
private lemma fv'_substAll_le [ProgramSpec] {S : Set FV} {Δ U} (m : ModuleExpression Δ U) :
  ...
  induction m with
  | proc => intros; simp [substSimultaneous, fv']
  | var r =>
      intros Γ σ hσ
...
theorem fv_reduction_step [ProgramSpec] (m : ModuleExpression c t) (nn : ¬ Normal m) :
...
theorem fv_reduce [ProgramSpec] (m : ModuleExpression c t) : fv m ⊆ fv' m := by
  unfold fv
...
theorem fv_app [ProgramSpec] (a : ModuleExpression Γ (ModuleType.arr A B)) (b : ModuleExpression Γ A) :
...
-- theorem reduceFstPair {ctx mtya mtyb} (a : ModuleExpression ctx mtya) (b : ModuleExpression ctx mtyb) :
  -- reduce (ModuleExpression.pair a b).fst = reduce a := rfl
