...
/-- EC's `module Correctness (S : CommitmentScheme)`, as a functor module: apply it to a
    ...
    No rearrangement needed: `CommitmentScheme.typeRep` (generated by `moduletype`, last-declared
    field outermost, unit-terminated) and `Correctness.main`'s holes tuple (`uses (gen, commit,
    verify)`, same convention) agree exactly, so the functor parameter is just `.var .zero`. -/
noncomputable def Correctness :
  Module.Arr CommitmentScheme (Module (procmod (Message) -> Bool)) :=
  (ModuleExpression.abs
    (.app (.procHoles (by trivial) Correctness.main) (.var .zero))).toModule
