...
/-- A `Footprint R` **collapses to `σ`** if a single deterministic `Rᶜ`-update fixes `σ` and sends
    every state into the (one-step) `R`-orbit of `σ`.  The `Footprint` analogue of
    `DetermFootprint.HasOrbitCollapse`: available for lens-derived `R` (a complement "reset"),
    absent for an abelian bicommutant-closed `R`.  It is the faithfulness witness that lets a
    confined program preserve the whole `R`-complement content (`global_getter`). -/
def Footprint.HasOrbitCollapse {m : Type} (R : Footprint m) (σ : m) : Prop :=
  ∃ f : Function.End m, diracKer f ∈ Rᶜ.updates ∧ f σ = σ ∧
    ∀ s, ∃ u : Function.End m, diracKer u ∈ R.updates ∧ u σ = f s
