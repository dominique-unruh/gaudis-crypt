...
@[symm]
lemma symm {p q : ProgramDenotation s α} (h : ProgramDenotation.EquivModuloLens L p q) :
    ProgramDenotation.EquivModuloLens L q p :=
  fun F h_F σ => (h F h_F σ).symm

lemma trans {p q r : ProgramDenotation s α}
    (h1 : ProgramDenotation.EquivModuloLens L p q) (h2 : ProgramDenotation.EquivModuloLens L q r) :
    ProgramDenotation.EquivModuloLens L p r :=
...
/-- Bind with the SAME prefix and equivalent continuations: if `∀ a, k a ≈_L k' a`,
    then `p >>= k ≈_L p >>= k'`. -/
lemma bind_eq_p
    {p : ProgramDenotation s α} {k k' : α → ProgramDenotation s β}
...
lemma wp_eq {p q : ProgramDenotation s α} (h : ProgramDenotation.EquivModuloLens L p q)
    (F : α × s → ENNReal) (h_F : IgnoresLens L F) (σ : s) :
    p.wp F σ = q.wp F σ := h F h_F σ

end ProgramDenotation.EquivModuloLens
...
lemma loop_n_congr {s γ : Type} [DecidableEq γ] {L : Lens γ s}
  ...
  induction n with
  | zero => exact ProgramDenotation.EquivModuloLens.refl _
  | succ n ih =>
...
lemma loop_n_then_congr {s γ : Type} [DecidableEq γ] {L : Lens γ s}
    ...
    ProgramDenotation.EquivModuloLens L
        ...
        (loop_n n body' >>= fun _ : Unit => final') :=
  ProgramDenotation.EquivModuloLens.bind
