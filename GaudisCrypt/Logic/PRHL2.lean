...
* `Program.prhl.to_prhl2` (forward) recovers the distribution-equality marginals via
  `Coupling.map_fst/map_snd`, and `satisfies_of_range` turns the range support into the pointwise one.
* `Program.prhl2.to_prhl` (backward) reads the pointwise `satisfies` back as the range form because
